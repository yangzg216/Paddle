...
static int BuildFusion(Graph* graph, const std::string& name_scope,
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    for(int i = 0; i < num_layers; i++){
      ...
      w0_data = w0_tensor->mutable_data<float>(platform::CPUPlace());
      ...
      */
      ...
      //const std::string& tmp_name = patterns::UniqueKey("new_batchedgemm_out_tmp_"+ std::to_string(i));
      //VarDesc fc_out_var_desc(tmp_name); //avoid exist
      //fc_out_var_desc.SetPersistable(false);
      
      //auto* fc_out_var = g->CreateVarNode(&fc_out_var_desc);
      //fc_output_vars[i] = fc_out_var;
      //fc_output_names[i] = tmp_name;


      const std::string& tmp_out_name = patterns::UniqueKey("new_batchedgemm_out_tmp_"+ std::to_string(i));
      fc_output_names[i] = tmp_out_name;
      ...
      OpDesc split_op_desc(orig_ops[0]->Op()->Block());
      split_op_desc.SetType("split");
      if (i == num_layers -1)
      {
        int64_t _n = w_shape[1];
        //transpose2
        const std::string& transpose_out_name = patterns::UniqueKey("new_transpose_out");
        //VarDesc transpose_out_var_desc(transpose_out_name);
        //transpose_out_var_desc.SetPersistable(false);
        //auto* transpose_out_var = g->CreateVarNode(&transpose_out_var_desc);
        auto* transpose_out_var_desc =
          orig_ops[0]->Op()->Block()->Var(transpose_out_name);
        transpose_out_var_desc->SetPersistable(false);
        std::vector<int64_t> tmp_input_shape = input_vars_input[0]->Var()->GetShape();
        transpose_out_var_desc->SetShape({tmp_input_shape[0], num_columns, _n}); //bc,batch,n
        transpose_out_var_desc->SetDataType(input_vars_input[0]->Var()->GetDataType());
        auto* transpose_out_var = g->CreateVarNode(transpose_out_var_desc);


        OpDesc transpose_op_desc(orig_ops[0]->Op()->Block());
        transpose_op_desc.SetType("transpose2");
        transpose_op_desc.SetInput("X", {fc_output_names[i]});
        std::vector<int> transpose_shape = {1,0,2};
        transpose_op_desc.SetAttr("axis", transpose_shape);//from [bc, batch, embdim] to [batch, bc, embdim]
        transpose_op_desc.SetOutput("Out", {transpose_out_name});
        auto* new_transpose_op = graph->CreateOpNode(&transpose_op_desc);
        IR_NODE_LINK_TO(fc_out_var, new_transpose_op);
        IR_NODE_LINK_TO(new_transpose_op, transpose_out_var);
        
        const std::string& reshape_out_name = patterns::UniqueKey("new_reshape_out");
        auto* reshape_out_var_desc =
            orig_ops[0]->Op()->Block()->Var(reshape_out_name);
        reshape_out_var_desc->SetPersistable(false);
        //std::vector<int64_t> tmp_input_shape = input_vars_input[0]->Var()->GetShape();
        reshape_out_var_desc->SetShape({tmp_input_shape[0], num_columns * _n}); //from batch,bc,n to batch,bc*n
        reshape_out_var_desc->SetDataType(input_vars_input[0]->Var()->GetDataType());
        auto* reshape_out_var = g->CreateVarNode(reshape_out_var_desc);
        
        OpDesc reshape_op_desc(orig_ops[0]->Op()->Block());
        reshape_op_desc.SetType("reshape2");
        reshape_op_desc.SetInput("X", {transpose_out_name});
        std::vector<int> reshape_shape = {static_cast<int>(tmp_input_shape[0]), static_cast<int>(num_columns * _n)};
        reshape_op_desc.SetAttr("shape", reshape_shape);
        reshape_op_desc.SetOutput("Out", {reshape_out_name});
        auto* new_reshape_op = graph->CreateOpNode(&reshape_op_desc);
        IR_NODE_LINK_TO(transpose_out_var, new_reshape_op);
        IR_NODE_LINK_TO(new_reshape_op, reshape_out_var);
        /* split后的reshape
        //reshape2
        std::vector<std::string> reshape_in_names(num_columns);
        std::vector<Node*> reshape_in_vars(num_columns);
        std::vector<Node*> reshape_ops(num_columns);
        for(int j = 0;j<num_columns;j++){
          reshape_in_names[j] = patterns::UniqueKey("new_reshape_in");
          //VarDesc reshape_in_var_desc(reshape_in_names[j]);
          //reshape_in_var_desc.SetPersistable(false);
          //auto* reshape_in_var = g->CreateVarNode(&reshape_in_var_desc);
          //reshape_in_vars[j] = reshape_in_var;

          auto* reshape_in_var_desc =
            orig_ops[0]->Op()->Block()->Var(reshape_in_names[j]);
          reshape_in_var_desc->SetPersistable(false);
          std::vector<int64_t> tmp_input_shape = input_vars_input[0]->Var()->GetShape();
          reshape_in_var_desc->SetShape({tmp_input_shape[0], 1, w_shape[1]}); //bc,batch,n
          reshape_in_var_desc->SetDataType(input_vars_input[0]->Var()->GetDataType());
          auto* reshape_in_var = g->CreateVarNode(reshape_in_var_desc);
          reshape_in_vars[j] = reshape_in_var;
          ...
          OpDesc reshape_op_desc(orig_ops[0]->Op()->Block());
          reshape_op_desc.SetType("reshape2");
          reshape_op_desc.SetInput("X", {reshape_in_names[j]});
          std::vector<int> reshape_shape = {-1, int(w_shape[1])};
          reshape_op_desc.SetAttr("shape", reshape_shape);
          reshape_op_desc.SetOutput("Out", {output_names[i* num_columns + j]});
          auto* new_reshape_op = graph->CreateOpNode(&reshape_op_desc);
          reshape_ops[j] = new_reshape_op;
        }
        */
        ...
        split_op_desc.SetInput("X", {reshape_out_name});
        ...
        IR_NODE_LINK_TO(reshape_out_var, new_split_op);

        for(int j = 0;j<num_columns;j++){
          //IR_NODE_LINK_TO(new_split_op, reshape_in_vars[j]);
          //IR_NODE_LINK_TO(reshape_in_vars[j], reshape_ops[j]);
          //IR_NODE_LINK_TO(reshape_ops[j], output_vars[i* num_columns + j]);
